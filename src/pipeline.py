...
def run_etl_pipeline():
    ...
    if not os.path.exists(RAW_DATA_PATH):
        logger.error(f"Critical execution failure: Missing raw data asset at {RAW_DATA_PATH}")
    ...
    cleaned_rows = len(df)
    logger.info(f"Data transformations complete. Filtered out {initial_rows - cleaned_rows} anomalous records.")
    ...
    logger.info(f"Loaded records cleanly into indexed warehouse storage layer: {DB_PATH}")
