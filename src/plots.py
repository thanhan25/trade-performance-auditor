...
def generate_performance_charts():
    logger.info("Extracting warehouse execution metrics for analytical visualization...")
    
    if not os.path.exists(DB_PATH):
        logger.error(f"Visualization aborted: Database target missing at reference path {DB_PATH}")
    ...
    logger.info(f"Success! High-resolution analytics charts exported to active portfolio layer: {OUTPUT_IMAGE_PATH}")
